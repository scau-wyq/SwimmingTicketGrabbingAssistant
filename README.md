...
也可以提前手动运行 `python auto_grab.py`：正式模式会先等待当天 **23:58**，这段时间不启动 mitmdump、不启用临时代理、不打开微信；到点后才开始上述准备流程，并在紧接着的零点发送预约请求。如果在 23:58–23:59 启动，则立即准备；如果准备时已错过本次零点，则退出，不顺延一天。每次运行只执行一轮，不会自动循环到次日。

启动时若发现上次异常留下的代理恢复快照，仍会先执行恢复，避免让错误代理持续到晚上；这不会启动抓包或微信。
...
# 正式预约：先等到 23:58 才获取 token，再等到紧接着的零点
python auto_grab.py
...
`--test` 和 `--wechat-only` 保持立即执行，不等待 23:58；`--manual` 正式运行仍先等待 23:58。等待期间可按 Ctrl+C 退出。
